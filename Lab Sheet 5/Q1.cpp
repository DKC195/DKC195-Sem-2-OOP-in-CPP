...
another class for Operations.
B. Make the other three functions to work as a bridge between the classes for
multiplication, division, and subtraction.
C. Also write a small program to demonstrate that all the member functions of
one class are the friend functions of another class if the former class is made
friend to the latter.
Make least possible classes to demonstrate all the above in a single program
without conflict. */
#include <iostream>
using namespace std;
...
void multi(Operations O, Coordinate C) {
  C.x1 = O.x * C.x;
  C.y1 = O.y * C.y;
  cout << "Product of Coordinates (x, y): (" << C.x1 << ", " << C.y1 << ")"
       << endl;
}
...
void div(Operations O, Coordinate C) {
  C.x1 = O.x / C.x;
  C.y1 = O.y / C.y;
  cout << "Quotient of Coordinates (x, y): (" << C.x1 << ", " << C.y1 << ")"
       << endl;
}
...
void sub(Operations O, Coordinate C) {
  C.x1 = O.x - C.x;
  C.y1 = O.y - C.y;
  cout << "Difference Coordinates (x, y): (" << C.x1 << ", " << C.y1 << ")"
       << endl;
}
