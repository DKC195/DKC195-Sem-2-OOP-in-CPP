...
function toFahrenheit() in class Celsius that converts to Fahrenheit scale and
returns the value. */
#include <iostream>
using namespace std;
...
public:
  float toFahrenheit(float temp) { return (temp * multi / div) + base; }
};
...
public:
  float toCelcius(float temp) { return ((temp - base) * multi / div); }
};
